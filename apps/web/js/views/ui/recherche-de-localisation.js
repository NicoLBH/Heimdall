...
import { renderCarteAPointer, brancherLaCarteAPointer } from "./carte-a-pointer.js";
...
function renderFenetre(etat) {
  const { depart, retenue, centre, zoom, mapUrl, mapLoading, echec, occupee } = etat;
    ...
    <div class="fichiers-saisie" role="dialog" aria-modal="true" aria-label="Recherche de localisation">
      <div class="fichiers-saisie__boite recherche-localisation">
        <header class="fichiers-saisie__tete">
          <b>${svgIcon("location", { className: "octicon" })} Où est ce projet, exactement ?</b>
        ...
        <p class="recherche-localisation__lead">
          Tapez une adresse, ou seulement la commune. Puis déplacez la carte jusqu'à reconnaître
          le terrain, et posez le point — <b>appui long</b>, ou le bouton du viseur.
        </p>
        ...
        <div class="recherche-localisation__retenue">
          ${
            occupee
          ...
          ${
            ...
            // mètres sans changer de commune — c'est un versant, donc une
            // altitude, donc une cote hors gel.
        ...
        <footer class="fichiers-saisie__pied">
          ...
          <button type="button" class="gh-btn gh-btn--primary" data-recherche-retenir
            ${retenue && localisationCalculable(retenue) && !occupee ? "" : "disabled"}>
...
export async function chercherUneLocalisation({ depart = null } = {}) {
  ...
  return new Promise((resoudre) => {
    ...
    const dessiner = () => {
      ...
      hote.querySelector("[data-recherche-retenir]")?.addEventListener("click", () => fermer(etat.retenue));

      brancherLaSaisieDAdresse(hote, {
        nom: SAISIE,
...
async function rafraichirLaCarte(etat, dessiner) {
  ...
  if (cle === etat.mapCle && etat.mapUrl) return;
  etat.mapCle = cle;
  etat.mapLoading = true;
  dessiner();

  try {
    etat.mapUrl = await fetchGoogleMapsPlaceEmbedUrl({ latitude, longitude, zoom: etat.zoom, mapType: "satellite" });
  } catch {
    etat.mapUrl = "";
