...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("[delete-account] request received", {
      ...
      authorizationPreview: maskToken(authorizationHeader.replace(/^Bearer\s+/i, "")),
    ...
    if (!authHeader) {
      console.warn("[delete-account] missing authorization header", { requestId });
    ...
    if (getUserError || !user?.id) {
      console.warn("[delete-account] auth.getUser failed", {
        requestId,
    ...
    console.log("[delete-account] authenticated user", {
      requestId,
      userId: user.id,
    ...
    console.log("[delete-account] payload received", {
      requestId,
      hasIdentityInput: Boolean(safeString(body?.identityInput)),
      confirmationMatches: normalizeText(body?.confirmationText) === DELETE_ACCOUNT_CONFIRMATION_TEXT
    });
    ...
    const normalizedIdentityInput = normalizeText(identityInput);
    console.log("[delete-account] identity check", {
      requestId,
      acceptedIdentities,
      normalizedIdentityInput
    });

    if (!acceptedIdentities.includes(normalizedIdentityInput)) {
...
function maskToken(value: unknown): string {
  ...
  if (!text) return "";
  if (text.length <= 12) return `${text.slice(0, 4)}…${text.slice(-2)}`;
...
function redactHeaders(headers: Record<string, string>): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(headers)) {
    if (/authorization/i.test(key)) {
      redacted[key] = `Bearer ${maskToken(String(value).replace(/^Bearer\s+/i, ""))}`;
      continue;
    }
    if (/apikey/i.test(key)) {
      redacted[key] = maskToken(value);
      continue;
    }
    redacted[key] = value;
  }
